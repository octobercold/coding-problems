...
for (const line of lines) {
    ...
    for (let i = 1; i < bridge.length; i++) {
        console.log(`looking to move knot: ${i} with current coordinates ${bridge[i].x}, ${bridge[i].y}`);
        for (let j = 0; j < steps - 1; j++) {
            const newCurrentKnotCoordinate = moveKnot(bridge[i], newH);
            console.log(`moved knot ${i} to the new coordinate ${newCurrentKnotCoordinate.x},${newCurrentKnotCoordinate.y}`);
        }
        console.log(`${bridge[i].x},${bridge[i].y} is going to be a target for the next knot`);
        newH = bridge[i];
    }
}
console.log(bridge);
//# sourceMappingURL=day9-2.js.map