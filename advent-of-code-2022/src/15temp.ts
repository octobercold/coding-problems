...
const generateCoverage = (
    ...
    console.log(
        `distance: ${distance}, left: ${left}, right: ${right}, top: ${top}, bottom: ${bottom}`
    );
...
const noBeaconHere = (): number => {
    let size = rowCoverage.size;
    for (const beacon of beacons) {
        if (rowCoverage.has(beacon)) size--;
    }
    return size;
};

console.log(noBeaconHere());
