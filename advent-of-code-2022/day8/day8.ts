...
const partTwo = (): number => {
    const checkTop = (starti: number, startj: number): number => {
        const treeHeight = trees[starti][startj];
        for (let i = starti - 1; i >= 0; i--) {
    ...
    const checkRight = (starti: number, startj: number): number => {
        const treeHeight = trees[starti][startj];
        for (let j = startj + 1; j < trees[starti].length; j++) {
    ...
    const checkDown = (starti: number, startj: number): number => {
        const treeHeight = trees[starti][startj];
        for (let i = starti + 1; i < trees.length; i++) {
            if (trees[i][startj] >= treeHeight) {
                return i - starti;
    ...
    const checkLeft = (starti: number, startj: number): number => {
        const treeHeight = trees[starti][startj];
        for (let j = startj - 1; j >= 0; j--) {
            if (trees[starti][j] >= treeHeight) {
                return startj - j;
